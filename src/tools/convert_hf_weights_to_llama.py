...
def write_model(model_path: str, model_size: str, output_base_path: str) -> None:
    """Write the model."""
    ...
    base = 10000.0
    inv_freq = (1.0 / (base ** (torch.arange(0, dims_per_head, 2).float() / dims_per_head))).to(dtype)
    ...
    def permute(w: torch.Tensor, n_heads: int = n_heads, dim1: int = dim, dim2: int = dim) -> torch.Tensor:
        """Permute the w."""
        return w.view(n_heads, 2, dim1 // n_heads // 2, dim2).transpose(1, 2).reshape(dim1, dim2)

    state_dict: List[Dict[str, torch.Tensor]] = [{} for _ in range(num_shards)]

    def insert(name: str, tensor: Union[List, torch.Tensor]) -> None:
        """Insert."""
        for i in range(num_shards):
            state_dict[i][name] = tensor[i].clone() if isinstance(tensor, list) else tensor

    def insert_chunk(name: str, tensor: torch.Tensor, dim: int) -> None:
        """Insert Chunk."""
    ...
    for i in tqdm(range(num_shards), desc="Saving checkpoint shards"):
        torch.save(state_dict[i], os.path.join(output_base_path, f"consolidated.{i:02d}.pth"))
