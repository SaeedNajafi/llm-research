...
def fsdp_auto_wrap_policy(model: torch.nn.Module, transformer_layer_name: str) -> Any:
    """Enable my auto wrap policy."""
    _ = model
...
def hsdp_device_mesh(replica_group_size: int, sharding_group_size: int, device: Optional[str] = None) -> Any:
    """Initializes a device mesh for use with Hybrid Sharding strategy in FSDP
    (HSDP) training.
    ...
    if world_size % sharding_group_size != 0:
        raise ValueError(f"World size {world_size} is not evenly divisible by " f"sharding group size {sharding_group_size}.")

    if (world_size // sharding_group_size) % replica_group_size != 0:
        raise ValueError(
            f"The calculated number of replica groups is not evenly divisible by " f"replica_group_size {replica_group_size}."
        )
