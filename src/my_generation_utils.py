...
class GenerateEncoderDecoderOutput(ModelOutput):
    """Outputs of encoder-decoder generation models, when using non-beam
    methods.
...
class MyGenerationMixin(GenerationMixin):
    """Subclass the original GenerationMixin to provide a generation function
    that mixes the probabilities between a student/teacher model or between a
    student/reference model.

    We only modify the generate and the _sample functions from the
    ...
    def generate(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            mix_in_model (`PreTrainedModel`, *optional*):
                A teacher model which will be used to adjust the distribution of the student sampler.
                (https://arxiv.org/abs/2306.08543)
            mix_in_alpha (`float`, *optional*):
                The alpha weight used to mix distribution of the teacher model with the student model.
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        r"""Generates sequences of token ids for models with a language modeling
        head using **multinomial sampling** and can be used for text-decoder,
        ...
        while self._has_unfinished_sequences(
            ...
            if mix_in_model is not None:
                m_outputs = mix_in_model(**m_model_inputs, return_dict=True)
            ...
            # Usually with EOS_Stopping_Criteria, we don't depend on the scores which is just from the student model.
