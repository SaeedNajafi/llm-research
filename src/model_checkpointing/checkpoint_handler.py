...
def save_model_and_optimizer_sharded(model: torch.nn.Module, rank: int, cfg: TRAIN_CONFIG, optim: Optional[torch.optim.Optimizer] = None):
...
def load_model_checkpoint(model: torch.nn.Module, rank: int, cfg: TRAIN_CONFIG):
...
def save_optimizer_checkpoint(model: torch.nn.Module,
                            optimizer: torch.optim.Optimizer, rank: int, cfg: TRAIN_CONFIG, epoch: int = 1) -> None:
...
def load_optimizer_checkpoint(model: torch.nn.Module, optimizer_checkpoint_path: Path, rank: int):
